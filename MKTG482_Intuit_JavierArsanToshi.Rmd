...
Before creating the Neural network we change the variables to factors 
...
Now wecreate the training and testing frames
```{r}
...
No we will create the Neural Network model and train it with the training set
```{r}
...
Next, I will create a prediction for the probability of purchase so we can compare it to the Logit model
```{r}
...
Now I will run the gainsplot to evaluete how the model performs in the test versus training
```{r}
